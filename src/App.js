...
import { SantaTeam, GrinchTeam } from "./utils";
...
const Arena = ({ p1, p2, isAttacking }) => {
  return (
    <div className="flex justify-between w-96 mx-auto mt-20">
      <div>
...
const Character = ({ character, isAttacking = false }) => {
  return (
    <div className={`text-center`}>
      <img
        className={`h-24 w-24 mb-3 mx-auto ${isAttacking && "animate-bounce"}`}
        src={character.imageURI}
        alt="grinch"
      />
      <h3 className="text-xl text-gray-900 font-medium mb-1 dark:text-white">
        #{character.name}
      </h3>
...
const App = () => {
  ...
  const runAttackAction = async (targetAddress) => {
    ...
    try {
      if (gameContract) {
        toast.info(
          "Transaction started: Attacking. It will be completed in ~1 min. Sit tight."
        );
        ...
        const pTwitter =
          allTwitters.find((e) => e.walletId === targetAddress)?.twitter ||
          "Player";
        const eTwitter =
          allTwitters.find((e) => e.walletId === enemy.wallet)?.twitter ||
          "Enemy";

        postLog({
          message: `${pTwitter} just attacked ${eTwitter} for ${characterNFT.attackDamage} points`,
          transaction: attackTxn,
        });
      }
    } catch (error) {
  ...
  const runHealingAction = async () => {
    ...
    try {
      if (gameContract) {
        toast.info(
          "Transaction started: Healing. It will be completed in ~1 min. Sit tight."
        );
        ...
        const pTwitter =
          allTwitters.find((e) => e.walletId === characterNFT.wallet).twitter ||
          "Player";

        postLog({
          message: `${pTwitter} just healed for ${characterNFT.healingPower} points`,
          transaction: healingTxn,
        });
      }
    } catch (error) {
  ...
  const postUser = (message) => {
    ...
    fetch("https://santawars-backend.herokuapp.com/user", requestOptions).then(
      (response) => {
  ...
  useEffect(() => {
    ...
    const onAttackComplete = async (newTargetHp, newPlayerHp) => {
      ...
      console.log(
        `AttackComplete: targetHp: ${targetHp} Player Hp: ${playerHp}`
      );
  ...
  const santas =
    allPlayers && allPlayers.filter((e) => SantaTeam.includes(e.name));
  const grinches =
    allPlayers && allPlayers.filter((e) => GrinchTeam.includes(e.name));
      ...
      <div className="connect-wallet-container text-center pb-8">
        ...
        <div className="w-1/2 mx-auto mb-10">
          ...
          <input
            ...
            class="block rounded-lg w-full p-3 mt-2 text-gray-800 bg-gray-200 appearance-none focus:outline-none focus:bg-gray-200 focus:shadow-inner"
            required
          />
        </div>
        ...
        <p className="mt-2">
          Deploy on Rinkeby testnet so you will not be using any real money
        </p>
  ...
  return (
    <div>
      <div className="h-8 bg-red-700 text-white text-center">
        <p className="pt-1">
          The team with the most survivers on Dec 25th, 2021 will be the winner
          🎁
        </p>
      ...
      >
        ...
        <div className="mb-8">
          <h1 className="text-lg mb-4">
            Santawars is an NFT game where you pick a side - Santa Team or
            Grinch pack
          </h1>
          ...
          <ul className="list-disc ml-8 mb-8">
            ...
            <li>
              The team with most standing people on Dec 25th will be the winner
            </li>
          </ul>
        </div>
      </Modal>
            ...
            <div className="text-center w-full flex justify-center items-center h-full">
              <div className>
                <div className="flex flex-col items-center pb-10">
                  <div>
                    {" "}
                    <Character character={characterNFT} />
                  </div>
                  ...
                  <div className="flex space-x-3 mt-4 lg:mt-6">
                    <button
                      ...
                      className="text-white bg-green-500 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-4 py-2 text-center inline-flex items-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"
                    >
                      Heal{" "}
                      <svg
                  ...
                  <button className="mt-4 mb-2" onClick={openModal}>
                    How to play
                  </button>
                  <a
          ...
          <div className="w-3/5 p-4 bg-gray-100">
            ...
            <div className="inline-flex shadow-sm rounded-md mt-4">
              <button
                ...
                className="rounded-l-lg border border-gray-200 bg-white text-sm font-medium px-4 py-2  hover:bg-gray-100 text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-500 dark:focus:text-white"
              >
                Santa Team
              </button>
              <button
                ...
                className="rounded-r-md border border-gray-200 bg-white text-sm font-medium px-4 py-2 text-gray-900 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-500 dark:focus:text-white"
              >
                Grinch Pack
              </button>
            </div>
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="px-6 py-4 text-left whitespace-nowrap text-sm font-medium">
                                    {!(
                                      (SantaTeam.includes(e.name) &&
                                        SantaTeam.includes(
                                          characterNFT.name
                                        )) ||
                                      (GrinchTeam.includes(e.name) &&
                                        GrinchTeam.includes(characterNFT.name))
                                    ) && (
                                        <button
                                          onClick={() => {
                                            runAttackAction(e.wallet);
                                          }}
              ...
              logs.map((e) => (
                <div>
                  <a
                    ...
                    href={`https://rinkeby.etherscan.io/address/${e.transaction}`}
                  >
                    {e.message}
                  </a>
                </div>
